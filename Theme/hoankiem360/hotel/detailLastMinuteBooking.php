	...
	.wr-header-bot{
 	...
 	<main class="detail_hotel container">
			<ul class="">
				...
				<li><a href=""> Đặt Phòng Cuối Ngày / </a></li>
 ...
 <style type="text/css">
    ...
    color: #ffffff;
				...
				<div class="row">
					...
					<div class="col-md-6">
						...
						<select name="typeRoom" id="typeRoom" onchange="tinhphi();">
							...
							<?php
								foreach($tmpVariable['data']['HotelManmo']['LastBooking']['LastBookingHour']['TypeRoom'] as $key => $BookingHour){
									foreach($tmpVariable['data']['HotelManmo']['listTypeRoom'] as $keytype => $typeRoom){
										if($typeRoom['TypeRoom']['id']== $key){?>
										?>
		...
		<div class="container">
			...
			<?php
				if(!empty($tmpVariable['data']['HotelManmo']['LastBooking']['LastBookingHour']['TypeRoom'])){ ?>
		                                            ...
		                                            <div class="detail_room row">
		                                                ...
		                                                <?php
		                                                    if(!empty($value['TypeRoom']['ngay_thuong']['gia_ngay'])){ ?>
		                                                            <div class="col-md-3">
		                                                                <p><?php echo number_format($BookingHour['gia_qua_dem']); ?>đ / <span>đêm</span></p>
    ...
    $('.btn_dp').attr('disable','disable');
       ...
       if(number_people!='' && number_room!='' && email!='' && phone!='' && typeRoom!='' && name!='' && type_register!='' && textNumberDate!='' && pricePay != '') {
        $.ajax({
            ...
            data: {
               idHotel:'<?php echo @$tmpVariable['data']['HotelManmo']['data']['Hotel']['id']; ?>',
...
function tinhthoigian(){
   ...
   if(date_start!='' && date_end!=''){
      ...
      if(date!=date_start){
         ...
         $('#textDiscount').html('Bạn phải chọn ngày ra dự kiến là ngày mai');
         $('#buttonSend').hide();
      }else if(ngay==1){
         $('#textDiscount').html('');
         $('#buttonSend').show();
